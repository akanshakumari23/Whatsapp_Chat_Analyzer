...
def conversation_starter(df):
    ...
    first_messages = df.groupby('date_only').first()

    starter_count = first_messages['user'].value_counts()

    return starter_count

def average_reply_time_pie(df):
    ...
    avg_reply = replies.groupby('user')['reply_time'].mean()
